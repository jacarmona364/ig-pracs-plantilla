...
void Textura::enviar()
{
   ...
   ////// DIAPOSITIVAS SECCIÓN 3.1: TEMA 3 (168-176). ////////
   ...
   // Especificar los parámetros de la textura // No nos dice cuales el enunciado de la Práctica
   // glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
   // glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
   // glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
   // glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);

   // Enviar los bytes de la textura a la GPU
   glTexImage2D(
      GL_TEXTURE_2D,      // Tipo de textura
      0,                  // Nivel de detalle (mipmap), nivel base
      GL_RGB,             // Formato de píxeles de entrada
      ancho,              // Ancho de la textura
      alto,               // Alto de la textura
      0,                  // Borde (siempre 0)
      GL_RGB,             // Formato de píxeles de salida en la GPU
      GL_UNSIGNED_BYTE,   // Tipo de datos de los píxeles
      imagen              // Puntero a los datos de la imagen
   );
...
void Textura::activar(  )
{
   ...
   // Activar la textura en el cauce gráfico
   cauce->fijarEvalText(true, ident_textura); // Activar el uso de texturas
   cauce->fijarTipoGCT(modo_gen_ct, coefs_s, coefs_t); // Configurar el modo de generación de coordenadas
