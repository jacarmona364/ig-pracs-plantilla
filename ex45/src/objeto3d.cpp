...
bool Objeto3D::cuandoClick( const glm::vec3 & centro_wc ) 
{
   ...
   // ...
   if (escena->camaraActual() != nullptr) {
      // Hacer que la cámara apunte al punto central de este objeto
      escena->camaraActual()->mirarHacia(centro_wc);
      cout << "La cámara ahora apunta al punto central del objeto: " << nombre_obj << endl;
   } else {
      cout << "Error: No se pudo obtener la cámara actual de la escena." << endl;
   }
