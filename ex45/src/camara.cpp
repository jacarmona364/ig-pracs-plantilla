...
void Camara3Modos::desplRotarXY( const float da, const float db )
{
   switch( modo_actual )
   {
      case ModoCam::examinar :
      {
         ...
         // .....
         org_polares[0] += da*180; // Actualizar el ángulo de rotación en el eje Z (latitud)
         org_polares[1] += db*180; // Actualizar el ángulo de rotación en el eje Y (longitud)
      ...
      case ModoCam::prim_pers_rot :
      {
         ...
         // .....
         org_polares[0] += da; // Actualizar el ángulo de rotación en el eje Z (latitud)
         org_polares[1] += db; // Actualizar el ángulo de rotación en el eje Y (longitud)

         // Calcular las nuevas coordenadas cartesianas y el vector de desplazamiento
         // punto_atencion = punto_atencion - (Cartesianas(org_polares)-org_cartesianas);
         // org_cartesianas = (Cartesianas(org_polares));
         glm::vec3 nueva_posicion = punto_atencion + Cartesianas(org_polares);
         glm::vec3 desplazamiento = nueva_posicion - (punto_atencion + org_cartesianas);

         // Restarle al punto de atención el vector de desplazamiento
         punto_atencion += desplazamiento;

         // Actualizar las coordenadas cartesianas
         org_cartesianas = Cartesianas(org_polares);
      ...
      case ModoCam::prim_pers_despl :
      {
         ...
         punto_atencion += eje[0] * da + eje[1] * db; 
         break ;
      }
   }
   matrices_actualizadas = false ;
...
void Camara3Modos::moverZ( const float dz )
{
   ...
   switch( modo_actual )
   {
      case ModoCam::examinar :
      {
         ...
         float r_min = 0.2,
               rc = 0.04;
         org_polares[2] = r_min + (org_polares[2] - r_min)*pow(1.0+rc,dz);
         org_cartesianas = Cartesianas(org_polares);
      ...
      case ModoCam::prim_pers_rot :
      {
         // COMPLETAR: práctica 5: mover en Z la cámara en modo 'primera persona rot'
         //
         // desplazar el punto de atención 'dz' unidades en el eje Z
         // nota: los ejes no cambian
         // .....
         punto_atencion += eje[Z] * dz;
         break ;
      }
      case ModoCam::prim_pers_despl :
      {
...
void Camara3Modos::mirarHacia( const glm::vec3 & nuevo_punto_aten )
{
   ...
   org_cartesianas += (punto_atencion - nuevo_punto_aten);
   org_polares = Esfericas(org_cartesianas);
