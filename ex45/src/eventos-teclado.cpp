...
void FGE_PulsarLevantarTecla( GLFWwindow* window, int key, int scancode, int action, int mods )
{
   ...
   if ( glfwGetKey( window, GLFW_KEY_L) == GLFW_PRESS )
   {
      ...
      if (ProcesaTeclaFuenteLuz(escena->colFuentes(), key)) {
         apl->revisualizar_escena = ProcesarTeclaAnimacion(objeto, key);
      }
   ...
   if ( glfwGetKey( window, GLFW_KEY_A ) == GLFW_PRESS )
   {
      ...
      apl->revisualizar_escena = ProcesarTeclaAnimacion(objeto, key); // DUDA: Así está correcto?
   ...
   switch ( key )
   {
      ...
      //
      case GLFW_KEY_Y:
         if (!apl->visualizar_fbo) {
            apl->visualizar_fbo = true;
            cout << "Visualizar FBO" << endl;
         } else {
            cout << "NO visualizar FBO" << endl;
            apl->visualizar_fbo = false;
         }
      break;
