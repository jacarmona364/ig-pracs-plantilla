...
bool AplicacionIG::seleccion( int x, int y ) 
{
   ...
   // (5) Si el ident del pixel es 0, imprimir mensaje y terminar (devolver 'false')
   // .......
   if (ident == 0) {
   ...
   //
   // .......
   Objeto3D *objetoSeleccionado;
   glm::vec3 centro;

   if (escena->objetoActual()->buscarObjeto(ident, glm::mat4(1.0f), &objetoSeleccionado, centro))
   {
      // Si se encuentra, ejecutar el método 'cuandoClick' del objeto encontrado y devolver el mismo valor devuelto por 'cuandoClick'
      return objetoSeleccionado->cuandoClick(centro);
   }
